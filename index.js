...
function getFinals(fifaData) {
 const FinalStage = fifaData.filter((final) => final.Stage == "Final");
 return FinalStage;
...
function getWinners(fifaData, callback) {
 return callback(fifaData).map(item => item [`Home Team Goals`] > item [`Away Team Goals`]? item[`Home Team Name`] : item[`Away Team Name`]);
}
...
const worldCupWin = data.reduce(function(accumulator, item){
    return accumulator + item[`worldCupWin`]
});
